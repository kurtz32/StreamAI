...
const MovieCard: React.FC<MovieCardProps> = ({ movie, onClick, className }) => {
  // Extract dimensions from className or use defaults for the image container
  // We apply the sizing classes to the image container to maintain the visual rhythm
  const imageClass = className || 'h-36 min-w-[160px] md:h-44 md:min-w-[280px]';
  ...
  return (
    ...
    >
      ...
      <div className="px-1 w-full max-w-[160px] md:max-w-[280px]">
         ...
         <div className="flex items-center gap-2 text-[11px] text-gray-400 font-medium mt-1">
           ...
           <span className="border border-gray-600 px-1 rounded-sm text-[10px]">{movie.rating}</span>
         </div>
